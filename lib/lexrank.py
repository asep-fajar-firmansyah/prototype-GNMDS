"""Lexrank Lib,
Lexrank is an unsupervised approach to text summarization on graph-based centrality scoring of sentences.
this code modified from https://github.com/asep-fajar-firmansyah/summarization/blob/master/lexrank.py.
"""
...
def lexrank_graph(similarity_matrix, cos_th, alpha):
    ...
    return lexrank, graph


def lexrank(sentences, word_thr=100, sent_split=False, word_split=False, stemming=False, cos_th=0, alpha=0.9):
    # sentence_tokenize
    if sent_split:
    ...
    for num, score in sorted(lexrank.items(), key=lambda x:x[1], reverse=True):
        ...
        if word_counter+length <= word_thr:
            ...
            if word_counter == 0:
                ...
                break
    return lexrank_sentences, lexrank_num, lexrank, graph
